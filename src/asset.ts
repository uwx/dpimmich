...
import { type Matcher, watch as watchFs } from 'chokidar';
...
import { type BaseOptions, Batcher, authenticate, crawl, sha1 } from './utils.js';
...
export interface UploadOptionsDto {
    ...
    watch?: boolean;
    jsonOutput?: boolean;
    backdateMap?: Record<string, Date>;
}
...
export const uploadFiles = async (
    files: string[],
    { dryRun, concurrency, progress, backdateMap }: UploadOptionsDto,
): Promise<Asset[]> => {
    if (files.length === 0) {
    ...
    const queue = new Queue<string, AssetMediaResponseDto>(
        async (filepath: string) => {
            ...
            const response = await uploadFile(filepath, stats, backdateMap?.[filepath]);
...
const uploadFile = async (input: string, stats: Stats, backdate?: Date): Promise<AssetMediaResponseDto> => {
    ...
    formData.append('fileCreatedAt', (backdate ?? stats.mtime).toISOString());
    formData.append('fileModifiedAt', (backdate ?? stats.mtime).toISOString());
